...
namespace CSC543FinalProject
{
    ...
    public class ParallelMapReduce
    {
        ...
        {
            Console.WriteLine("\n\n" + ("ParallelForEach - MapReduce" + " " + new string('=', 115)).Substring(0, 115));
            Program.HaltIfDebug();
            ...
            int multiplier = 200;
            string fileText = GetFileText(multiplier);
            ...
            // Map the words
            foreach(string wordBlock in ProduceWordBlocks(fileText))
            {
                // Split the block into words
                string[] words = wordBlock.Split(' ', StringSplitOptions.RemoveEmptyEntries);

                // Cleanup each word and save it
                StringBuilder wordBuffer = new StringBuilder();
                List<string> wordsList = new List<string>();
                foreach (string word in words)
                {
                    // Remove all spaces and punctuation other than apostrophes and hyphens
                    foreach (char c in word)
                    {
                        if (char.IsLetterOrDigit(c) || c == '\'' || c == '-')
                            wordBuffer.Append(c);
                    }
                    // ave the word in a list
            ...
            Parallel.ForEach<string, ConcurrentBag<string>>(ProduceWordBlocks(fileText)                      // input enumerator
                                                 ...
                                                 {   
                                                     ...
                                                     // Cleanup each word and save it locally
                                                     StringBuilder wordBuffer = new StringBuilder();
                                                     foreach (string word in words)
                                                     {   
                                                         // Remove all spaces and punctuation other than apostrophes and hyphens
         ...
         */
        private static IEnumerable<string> ProduceWordBlocks(string fileText)
        {
         ...
         */
        private static string GetFileText(int multiplier)
        {
